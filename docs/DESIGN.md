...
> Design notes for **paid-order-invoice-service** — a runnable typescript example that Typed Node service that turns fulfilled e-commerce orders into stored invoice PDFs.
...
- **Thin client** — a ~30-line helper that owns the base URL, the auth header, and envelope unwrapping, so call sites stay readable (e.g. `infrai.pdf.generate(...)`).
- **Feature code** — the actual task: e-commerce invoice PDF.
- **Configuration** — the API key is read from the `INFRAI_API_KEY` environment variable; no secret is ever hard-coded.

## Capabilities used

- `pdf.generate` — mapped to `POST /v1/pdf/generate`.
